...
export class theexpanseSystemActor extends Actor {
    ...
    prepareData() {
        ...
        if (this.data.type === 'char') {
            ...
            const initiativeFocus = game.settings.get("theexpanse-system", "initiativeFocus");
            if (initiativeFocus) {
    ...
    prepareBaseData() {
        ...
        if (this.data.img == CONST.DEFAULT_TOKEN) this.data.img = CONFIG.theexpanseSystem.actorIcons[actorData.type];

        // Check if split Armor is in use
        data.useBallisticArmor = game.settings.get("theexpanse-system", "useBallisticArmor");

        // Retrieve wealth mode
        data.useResource = data.useIncome = data.useCurrency = data.useCoins = false;
        const wealthMode = game.settings.get("theexpanse-system", "wealthType");
        switch (wealthMode) {
            case "income":
                data.useIncome = true;
                break;
    ...
    //         const gmScreenFlag = {gmScreenSheetClass: `theexpanse-system.theexpanseSystemSheetCharStatBlock`};
    //         this.data.flags = {
    //             ...this.data.flgas,
    ...
    _prepareBaseDataChar() {
        ...
        data.useConviction = game.settings.get("theexpanse-system", "useConviction");

        // Check if Toughness is in use
        data.useToughness = game.settings.get("theexpanse-system", "useToughness");

        // Check if Fatigue is in use
        data.useFatigue = game.settings.get("theexpanse-system", "useFatigue");

        // Check if Power Points is in use
        data.usePowerPoints = game.settings.get("theexpanse-system", "usePowerPoints");
        ...
        switch (data.fatigue.value) {
            case 0:
                data.fatigue.status = game.i18n.localize(CONFIG.theexpanseSystem.fatigueConditions.noFatigue);
            break;
            case 1:
                data.fatigue.status = game.i18n.localize(CONFIG.theexpanseSystem.fatigueConditions.winded);
            break;
            case 2:
                data.fatigue.status = game.i18n.localize(CONFIG.theexpanseSystem.fatigueConditions.fatigued);
            break;
            case 3:
                data.fatigue.status = game.i18n.localize(CONFIG.theexpanseSystem.fatigueConditions.exhausted);
            break;
            case 4:
                data.fatigue.status = game.i18n.localize(CONFIG.theexpanseSystem.fatigueConditions.dying);
            break;            
    ...
    _charItemModifiers() {
        ...
        const settingAbls = theexpanseSystem.abilities;
    ...
    _prepareBaseDataSpaceship() {
        ...
        data.hull.base = theexpanseSystem.spaceshipHull[data.sizeNumeric - 1];

        data.crew.min = theexpanseSystem.spaceshipCrew[data.sizeNumeric - 1].min
        data.crew.typical = theexpanseSystem.spaceshipCrew[data.sizeNumeric - 1].typ
    ...
    _addCrewPenalty(size, current, min) {
        ...
        const sizeArray = theexpanseSystem.spaceshipCrew;
    ...
    _addSizeMod(size, mod) {
        ...
        if (newSize > theexpanseSystem.spaceshipHull.length) newSize = theexpanseSystem.spaceshipHull.length;
        const newHull = theexpanseSystem.spaceshipHull[newSize - 1];
        return newHull;
    }
    ...
    _vehicleOperator(operatorData) {
        ...
        if (!operator) {
            ...
            let warning = game.i18n.format("theexpanse-system.WARNING.userNotAvailable", parts);
    ...
    handleConditions(condId, isChecked = null) {
        ...
        const condEffects = this.effects.filter(c => c.data.flags?.["theexpanse-system"]?.type === "conditions" && c.data.flags?.["theexpanse-system"]?.name === condId);
        ...
        if (isChecked && condEffects.length < 1) {
            const newEffect = CONFIG.statusEffects.filter(e => e.flags?.["theexpanse-system"]?.name === condId)[0];
