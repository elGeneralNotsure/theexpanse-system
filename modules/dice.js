...
export async function theexpanseRollCheck({
    event = null,
    actor = null,
    abl = null,
    itemRolled = null,
    ...
    if (!event.ctrlKey && event.altKey || selectAbl || event.type === "contextmenu") {
        extraOptions = await getTheExpanseRollOptions(itemRolled, {targetNumber: rollTN, selectAbl, rollVisibility});
        ...
        rollUserMod = extraOptions.theexpanseRollMod;
    ...
    if (resourceRoll === true) {
        ...
        const resSelected = game.settings.get("theexpanse-system", "wealthType");
        resName = game.i18n.localize(`theexpanse-system.${resSelected}`)
        partials.push({
            label: resName,
            value: rollData.resources
        });
        rollData.resourcesName = game.i18n.localize(`theexpanse-system.${resSelected}`);
    };
    ...
    if (abl !== null && abl !== "no-abl") {
        ...
        rollFormula += " + @ability";
        ablName = game.i18n.localize(`theexpanse-system.${abl}`);
        rollData = {
            ability: ablValue,
            ablCode: abl,
            focusId: null,
    ...
    if (rollType === "attack" && actor.data.data.attackMod !== 0) {
        ...
        partials.push({
            label: game.i18n.localize("theexpanse-system.bonus.attackMod"),
    ...
    if (actor?.data.data.testMod && actor?.data.data.testMod !== 0) {
        ...
        partials.push({
            label: game.i18n.localize("theexpanse-system.bonus.testMod"),
    ...
    if (rollUserMod) {
        ...
        rollFormula += " + @rollMod";
        partials.push({
            label: game.i18n.localize("theexpanse-system.setTheExpanseRollMod"),
            value: rollUserMod
        });
    }
    ...
    if (vehicleHandling) {
        ...
        partials.push({
            label: game.i18n.localize("theexpanse-system.handling"),
    ...
    if (itemRolled !== null && typeof(itemRolled) !== "string") {
        ...
        if (itemRolled.data.data.itemMods) {
            if (itemRolled.data.data.itemMods.itemActivation.isActive) {
                ...
                partials.push({
                    label: game.i18n.localize("theexpanse-system.activationMod"),
    ...
    if (actor) {
        ...
        if (aim.active && !resourceRoll) {
            ...
            rollFormula += " + @aim";
            partials.push({
                label: game.i18n.localize("theexpanse-system.aim"),
                value: aim.value
            });
        ...
        if (atkDmgTradeOff && !resourceRoll) {
            ...
            partials.push({
                label: game.i18n.localize("theexpanse-system.penaltyToDamage"),
        ...
        if (armor.penalty > 0 && abl === "dex") {
            ...
            partials.push({
                label: game.i18n.localize("theexpanse-system.armorPenalty"),
                value: rollData.armorPenalty
            })
        };   
        
        // Check if Fatigue is configured
        const usingFatigue = game.settings.get("theexpanse-system", "useFatigue");
        ...
        if (usingFatigue) {
            if (fatigue.value > 0) {
                ...
                rollFormula += " + @fatigue";
                partials.push({
                    label: game.i18n.localize("theexpanse-system.fatigue"),
        ...
        if (guardUp.active && !resourceRoll) {
            ...
            rollFormula += " + @guardUp";
            partials.push({
                label: game.i18n.localize("theexpanse-system.guardUp"),
        ...
        if (!flavor) {
            if (rollType === "fatigue") {
                headerTerms.item = game.i18n.localize("theexpanse-system.fatigue");
                flavor = game.i18n.format("theexpanse-system.chatCard.rollGeneral", headerTerms);
            } else {
                if (itemRolled) {
                    headerTerms.item = itemRolled.name;
                    switch (itemRolled.type) {
                        case "weapon":
                            flavor = game.i18n.format("theexpanse-system.chatCard.rollAttack", headerTerms);
                            break;
                        case "power":
                            flavor = game.i18n.format("theexpanse-system.chatCard.rollPower", headerTerms);
                            break;
                        default:
                            flavor = game.i18n.format("theexpanse-system.chatCard.rollGeneral", headerTerms);
                            break;
    ...
    // Finally, the TheExpanse Roll!
    const theexpanseRoll = await new Roll(rollFormula, rollData).evaluate({async: true});

    // If rollTN is used, check if roll fails or succeed
    let isSuccess = null
    if (rollTN) {
        const rollMargin = theexpanseRoll.total - rollTN;
        if (rollMargin >= 0) {
            isSuccess = true;
        } else {
            isSuccess = false;
    ...
    const rollSummary = theexpanseRollChecker(theexpanseRoll, generateSP)
    let chatTemplate = "/systems/theexpanse-system/templates/rolls/base-theexpanse-roll.hbs";

    rollData = {
        ...rollData,
        // Informs card's color scheme
        colorScheme: `colorset-${game.settings.get("theexpanse-system", "colorScheme")}`,
        flavor,
        partials,
        actorId,
        isToken,
        isSuccess,
        roll: theexpanseRoll,
        theexpanseRollSummary: rollSummary,
        rollTN,
        focusId,
        rollType,
        user: game.user
    ...
    if (game.modules.get("dice-so-nice") && game.modules.get("dice-so-nice").active) {
        const stuntDieColorset = game.settings.get("theexpanse-system", "stuntSoNice");
...
async function getTheExpanseRollOptions(itemRolled, data = {}) {
    // Ve se item rolado e arma, poder ou null/outro, 

    const template = "/systems/theexpanse-system/templates/rolls/theexpanse-roll-settings.hbs"
    const type = itemRolled ? itemRolled.type : null;

    if (data.selectAbl) {
        const abilitiesPool = CONFIG.theexpanseSystem.abilitiesSettings[game.settings.get("theexpanse-system", "abilitySelection")];
        let abilitiesArray = []
    ...
    return new Promise(resolve => {
        const data = {
            title: game.i18n.localize("theexpanse-system.theexpanseRollOptions"),
            content: html,
            buttons: {
                normal: {
                    label: game.i18n.localize("theexpanse-system.roll"),
                    callback: html => resolve(_processTheExpanseRollOptions(html[0].querySelector("form")))
                },
                cancel: {
                    label: game.i18n.localize("theexpanse-system.cancel"),
...
function _processTheExpanseRollOptions(form) {

    const modifiers = ["theexpanseRollMod", "atkDmgTradeOff", "rollTN", "selectedAbl"];
    let rollOptions = {}
...
export function theexpanseRollChecker(theexpanseRoll, generateSP) {
    const die1 = theexpanseRoll.dice[0].results[0].result;
    const die2 = theexpanseRoll.dice[0].results[1].result;
    const die3 = theexpanseRoll.dice[1].results[0].result;
...
async function getDamageRollOptions(addFocus, stuntDmg) {
    ...
    const template = "/systems/theexpanse-system/templates/rolls/dmg-roll-settings.hbs"
    // const type = itemRolled ? itemRolled.type : null;
    ...
    return new Promise(resolve => {
        const data = {
            title: game.i18n.localize("theexpanse-system.damageOptions"),
            content: html,
            buttons: {
                normal: {
                    label: game.i18n.localize("theexpanse-system.roll"),
...
export async function vehicleDamage ({
    ...
    // };
    let messageData = {
        flavor: `${vehicle.data.name} | ${game.i18n.localize(`theexpanse-system.${damageSource}`)}`,
        speaker: ChatMessage.getSpeaker()
    };
    ...
    if (addRam) {
        ...
        messageData.flavor += ` | ${game.i18n.localize("theexpanse-system.ram")}`;
    };
    ...
    if (stuntDie !== null) {
        ...
        messageData.flavor += ` | ${damageToString(stuntDie)} ${game.i18n.localize("theexpanse-system.stuntDie")}`; 
    }
    ...
    if (stuntDamage && stuntDamage !== 0) {
        ...
        messageData.flavor += ` | +${stuntDmgDice} ${game.i18n.localize("theexpanse-system.stunts")}`;             
    };
...
export async function itemDamage({
    ...
    if (item.isOwned) {
        ...
        if (item.data.data.hasDamage) {
            messageData.flavor += ` | ${game.i18n.localize(`theexpanse-system.${item.data.data.dmgType}`)} | ${game.i18n.localize(`theexpanse-system.${item.data.data.dmgSource}`)}`;
        };
        // Add Healing Flavor text if applicable
        if (item.data.data.hasHealing) {
            messageData.flavor += ` | ${game.i18n.localize(`theexpanse-system.item.healing`)}`;
        };
        ...
        if (dmgAbl !== null && dmgAbl !== "no-abl") {
            ...
            messageData.flavor += ` | ${damageToString(ablMod)} ${game.i18n.localize("theexpanse-system." + dmgAbl)}`
        }
        ...
        if (atkDmgTradeOff) {
            ...
            messageData.flavor += ` | ${damageToString(Math.abs(atkDmgTradeOff))} ${game.i18n.localize("theexpanse-system.penaltyToDamage")}`;
        }
        ...
        if (item.data.data.itemMods.itemDamage.isActive) {
            ...
            messageData.flavor += ` | ${damageToString(itemDmg)} ${game.i18n.localize("theexpanse-system.itemDmgMod")}`;
        };
        ...
        if (item.actor.data.data.ownedBonus != null && item.actor.data.data.ownedBonus.actorDamage) {
            ...
            messageData.flavor += ` | ${damageToString(actorDmgMod)} ${game.i18n.localize("theexpanse-system.itemDmgMod")}`;
        };
        ...
        if (item.actor.data.data.allOutAttack.active) {
            ...
            messageData.flavor += ` | ${damageToString(allOutAttackMod)} ${game.i18n.localize("theexpanse-system.allOutAttack")}`;                
        };
        ...
        if (actorDmgMod !== 0) {
            ...
            messageData.flavor += ` | ${damageToString(actorDmgMod)} ${game.i18n.localize("theexpanse-system.bonus.actorDamage")}`;             
        };
