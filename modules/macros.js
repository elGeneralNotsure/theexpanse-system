export function rollOwnedItem(itemName, rollOptions = false) {
  ...
  if (!actor) return ui.notifications.warn(game.i18n.localize("theexpanse-system.WARNING.selectTokenMacro"));

  const itemRolled = actor ? actor.items.find(i => i.name === itemName) : null;
  if (!itemRolled) {return ui.notifications.warn(game.i18n.localize("theexpanse-system.WARNING.actorDontHaveValidItem"));}
 ...
 * @returns {Promise}
 */
export async function createTheExpanseMacro(data, slot) {
  // if (data.type !== "weapon") return;
  if (!("data" in data)) return ui.notifications.warn("You can only create macro buttons for owned Items");
  const item = data.data;

  // Create the macro command
  const command = `game.theexpanseSystem.rollOwnedItem("${item.name}", true);\n\n/*Change second argument to false to skip Roll Options*/`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { "theexpanseSystem.itemMacro": true }
    });
  }
