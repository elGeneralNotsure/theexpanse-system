...
export default class theexpanseSystemItemSheet extends ItemSheet {
    constructor(...args) {
        super(...args);
    ...
    static get defaultOptions() {        
        ...
        return mergeObject(super.defaultOptions, {
            height: 450,
            width: 500,
            classes: ["theexpanse-system", "sheet", "item", "colorset-second-tier"],
            tabs: [{
    ...
    get template() {
        return `systems/theexpanse-system/templates/sheets/${this.item.data.type}-sheet.hbs`;
    };
    ...
    getData(options) {
        ...
        data.config.wealthMode = game.settings.get("theexpanse-system", "wealthType");
        ...
        if (this.item.data.type === "shipfeatures") {
            ...
            for (let f = 0; f < data.config.featuresType.length; f++) {
                ...
                data.config.featuresTypeLocal.push({
                    key: feat,
                    name: game.i18n.localize(`theexpanse-system.spaceship.${feat}`)
                });
            }
    ...
    async _onAddBonus(event) {
        ...
        for (const mod in bonusList) {
            if (Object.hasOwnProperty.call(bonusList, mod)) {
                ...
                if (!b.selected) {
                    const modName = game.i18n.localize(`theexpanse-system.bonus.${mod}`);
        ...
        const template = `systems/theexpanse-system/templates/bonus-select-form.hbs`;
        ...
        return new Promise(resolve => {
            ...
            new Dialog(data, {classes: ["theexpanse-system-dialog", "bonus-select", "dialog"]}).render(true);
        });
    }
