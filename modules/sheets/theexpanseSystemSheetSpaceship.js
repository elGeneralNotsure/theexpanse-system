...
export default class theexpanseSpaceshipSheet extends ActorSheet {
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            height: 750,
            resizable: false,
            classes: ["theexpanse-system", "sheet", "spaceship"]
        });
    }
                ...
                if (passenger.data.type === "char") {
                    ...
                    passengerList.map( p => {
                        if (p.id === passengerData.id) {
                            ...
                            let warning = game.i18n.format("theexpanse-system.WARNING.alreadyOnboard", parts);
                    ...
                    const warning = game.i18n.localize("theexpanse-system.WARNING.vehicleIsNotPassenger");
                    ui.notifications.warn(warning);
                }
            })
        };
    ...
    _onRollDice(event){
        ...
        if (event.currentTarget.classList.contains("roll-hull")) {
            ...
            messageData.flavor += ` | ${game.i18n.localize("theexpanse-system.spaceship.hull")}`;
        }
    ...
    _onRollManeuver(event) {
        ...
        if (datum.passengerId === "crew") {
            passenger.name = game.i18n.localize("theexpanse-system.spaceship.crew");
            const crewAction = {
        ...
        let flavorText = "theexpanse-system.chatCard.maneuversVehicle";
        switch (datum.sysName) {
            case "command":
                flavorText = "theexpanse-system.chatCard.commandsVehicle";
                break;
            case "sensors":
                flavorText = "theexpanse-system.chatCard.operatesVehicle";
                break;
            case "damageControl":
                flavorText = "theexpanse-system.chatCard.backupVehicle";
                break;
            default:
                break;
        }
        ...
        if (system) {
            rollData.moreParts.push({
                value: system.total,
                description: game.i18n.localize(`theexpanse-system.spaceship.systemName.${datum.sysName}`)
            })
        }
