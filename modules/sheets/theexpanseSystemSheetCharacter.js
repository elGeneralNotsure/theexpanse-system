...
export default class theexpanseSystemSheetCharacter extends ActorSheet {
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            width: 680,
            height: 800,
            classes: ["theexpanse-system", "sheet", "char", "standard"],
            tabs: [{
    ...
    get template() {
        return `systems/theexpanse-system/templates/sheets/${this.actor.data.type}-sheet.hbs`;
    }

    get observerRoll () {
        return game.settings.get("theexpanse-system", "observerRoll");
    }
    ...
    getData(options) {
        ...
        data.config = CONFIG.theexpanseSystem;
        ...
        data.condEffects = data.effects.filter(e => e.flags?.["theexpanse-system"]?.type === "conditions");
        // ...from all other Effects
        data.effects = data.effects.filter(e => !e.flags?.["theexpanse-system"]?.type);
        data.effects = sortObjArrayByName(data.effects, `label`);
    
        // Retrieve Prefession/Ancestry settings
        data.ancestry = game.settings.get("theexpanse-system", "ancestryOpt");
        data.occupation = game.settings.get("theexpanse-system", "occupation");

        // Retrieve Health Mode (Health/Fortune)
        data.healthMode = game.settings.get("theexpanse-system", "healthMode");

        // Sheet color
        data.colorScheme = game.settings.get("theexpanse-system", "colorScheme");
    ...
    _getHeaderButtons() {
        ...
        const isFull = sheet === undefined || sheet === 'theexpanse-system.theexpanseSystemSheetCharacter';

        buttons = [
            {
              label: isFull ? game.i18n.localize("theexpanse-system.sheetBlock") : game.i18n.localize("theexpanse-system.sheetFull"),
    ...
    async _onToggleSheet(event) {
        ...
        let newSheet = 'theexpanse-system.theexpanseSystemSheetCharStatBlock'
        ...
        if (original != 'theexpanse-system.theexpanseSystemSheetCharacter') newSheet = 'theexpanse-system.theexpanseSystemSheetCharacter'
    ...
    _onAddEffect(event) {
        const newEffect = {
            label: game.i18n.localize("theexpanse-system.item.newItem"),
    ...
    _onRollResources(event) {
        ...
        Dice.theexpanseRollCheck(rollData);
    };
    ...
    focusContextMenu = [
        {
            name: game.i18n.localize("theexpanse-system.theexpanseRollOptions"),
            icon: '<i class="fas fa-dice"></i>',
            callback: e => {
                const focus = this.actor.items.get(e.data("item-id"));
                const ev = new MouseEvent('click', {altKey: true});
                Dice.theexpanseRollCheck({event: ev, itemRolled: focus, actor: this.actor});
            }
        },
        {
        ...
        {
            name: game.i18n.localize("theexpanse-system.settings.changeRollContext"),
            icon: '<i class="fas fa-exchange-alt"></i>',
            // TODO - try to add the Shift + Click rolling to GM inside this callback
            callback: e => {
                const focus = this.actor.items.get(e.data("item-id"));
                const ev = new MouseEvent('click', {});
                Dice.theexpanseRollCheck({event: ev, itemRolled: focus, actor: this.actor, selectAbl: true});
            }
        },
        {
