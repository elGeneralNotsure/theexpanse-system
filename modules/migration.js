...
export const migrateWorld = async function() {
  ui.notifications.info(`Applying THE EXPANSE System Migration for version ${game.system.data.version}. Please be patient and do not close your game or shut down your server.`, {permanent: true});
  ...
  for ( let a of game.actors.entities ) {
    try {
      ...
      err.message = `Failed THE EXPANSE System migration for Actor ${a.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let i of game.items.entities ) {
    try {
      ...
      err.message = `Failed THE EXPANSE System migration for Item ${i.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let s of game.scenes.entities ) {
    try {
      ...
      err.message = `Failed THE EXPANSE System migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  game.settings.set("theexpanse-system", "systemMigrationVersion", game.system.data.version);
  ui.notifications.info(`THE EXPANSE System Migration to version ${game.system.data.version} completed!`, {permanent: true});
};
...
export const migrateCompendium = async function(pack) {
  ...
  for ( let doc of documents ) {
    ...
    // Handle migration failures
      ...
      err.message = `Failed theexpanse-system system migration for entity ${doc.name} in pack ${pack.collection}: ${err.message}`;
      console.error(err);
    }
  }
...
export const migrateItemData = function(item) {
  const lastMigrationVer = game.settings.get("theexpanse-system", "systemMigrationVersion");
  const updateData = {};
