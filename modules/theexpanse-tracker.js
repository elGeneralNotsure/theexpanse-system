export class TheExpanseTracker extends Application {
	constructor(options = {}) {
		super(options)
	}

	get template() {
		return `systems/theexpanse-system/templates/theexpanse-tracker.hbs`;
	}
	...
	getData(options) {
		...
		data.hasSerendipity = game.settings.get("theexpanse-system", "serendipity");
		if (data.hasSerendipity) {
			const serData = game.settings.get("theexpanse-system", "serendipityValue");
			if (serData.actual > serData.max) {
				serData.actual = serData.max;
				game.settings.set("theexpanse-system", "serendipityValue", serData);
			};
			const sides = 6;
		...
		const compType = game.settings.get("theexpanse-system", "complication");
		data.hasComplication = (compType === "none") ? false : true;
		if (data.hasComplication) {
			const compData = game.settings.get("theexpanse-system", "complicationValue");
	...
	activateListeners(html) {
		...
		html.find("#theexpanse-tracker-drag").contextmenu(this._onRightClick.bind(this));

		// Set position
		let tracker = document.getElementById("theexpanse-tracker");
		const trackerPos = game.user.getFlag("theexpanse-system", "theexpanseTrackerPos");
	...
	_onRightClick(event) {
		const tracker = event.currentTarget.closest("#theexpanse-tracker");
		const original = CONFIG.theexpanseSystem.theexpanseTrackerPos;
		tracker.style.left = original.xPos;
		tracker.style.bottom = original.yPos;
		game.user.setFlag("theexpanse-system", "theexpanseTrackerPos", original);
	}

	_onClickComp(event) {
		event.preventDefault();
		const compData = game.settings.get("theexpanse-system", "complicationValue");
		if (event.currentTarget.classList.contains('refresh')) return game.settings.set("theexpanse-system", "complicationValue", {max: compData.max, actual: 0});
		let value;
		...
		game.settings.set("theexpanse-system", "complicationValue", compData);
	}

	async _onRollComp() {
		const compType = game.i18n.localize(`SETTINGS.comp${game.settings.get("theexpanse-system", "complication")}`);
		const flavor = game.i18n.format("theexpanse-system.chatCard.compRoll", {compType});
	...
	_dragElement(elmnt) {
		...
		if (document.getElementById("theexpanse-tracker-drag")) {
		  // if present, the header is where you move the DIV from:
		  document.getElementById("theexpanse-tracker-drag").onmousedown = dragMouseDown;
		} else {
		...
		function closeDragElement() {
			...
			game.user.setFlag("theexpanse-system", "theexpanseTrackerPos", trackerPos);
		}
	}
}